  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const requestData = {
        ...hoaDon,
        id: hoaDon._id,
        hopDong: typeof hoaDon.hopDong === 'object' ? hoaDon.hopDong._id : hoaDon.hopDong,
      ...
      <div className="bg-gray-50 p-3 md:p-4 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 text-xs md:text-sm">
          ...
          <div>
            ...
            <div className="font-medium text-blue-600 text-lg">{formatCurrency(hoaDon.tongTien)}</div>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
        <div className="space-y-2">
          ...
          <Input
            id="soTien"
            type="text"
            value={formatCurrency(hoaDon.tongTien)}
            disabled
        ...
        <div className="space-y-2">
          ...
          <Select value="chuyenKhoan" disabled>
            <SelectTrigger className="h-10 md:h-12 text-sm bg-gray-100">
          ...
          <div className="text-[10px] md:text-xs text-gray-500 mt-1">
            Chỉ hỗ trợ xác nhận thanh toán qua chuyển khoản
          </div>
        </div>
