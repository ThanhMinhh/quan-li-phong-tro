...
function ToaNhaForm({
  ...
  const [formData, setFormData] = useState({
    ...
    chuSoHuu: typeof toaNha?.chuSoHuu === 'object' && toaNha.chuSoHuu !== null
      ? (toaNha.chuSoHuu as any)._id
      : (toaNha?.chuSoHuu as string) || '',
  });
  const [chuNhaList, setChuNhaList] = useState<{ _id: string; ten: string; email: string }[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData = {
        ...
        ...(formData.chuSoHuu ? { chuSoHuu: formData.chuSoHuu } : {}),
      };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
      ...
      {/* Chủ sở hữu - chỉ hiển thị khi có danh sách */}
      {chuNhaList.length > 0 && (
        <div className="space-y-2">
          <Label htmlFor="chuSoHuu" className="text-sm">Chủ sở hữu</Label>
          <Select
            value={formData.chuSoHuu}
            onValueChange={(value) => setFormData(prev => ({ ...prev, chuSoHuu: value }))}
          >
            <SelectTrigger id="chuSoHuu" className="text-sm">
              <SelectValue placeholder="Chọn chủ sở hữu..." />
            </SelectTrigger>
            <SelectContent>
              {chuNhaList.map((cn) => (
                <SelectItem key={cn._id} value={cn._id}>
                  <span className="font-medium">{cn.ten}</span>
                  <span className="text-muted-foreground ml-2 text-xs">({cn.email})</span>
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
