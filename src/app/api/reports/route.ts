...
async function getRevenueReport(start: Date, end: Date, format: string) {
  ...
  const revenueByMonth = await HoaDon.aggregate([
    {
      $match: {
        ngayThanhToan: {
          $gte: start,
  ...
  const revenueByMethod = await HoaDon.aggregate([
    {
      $match: {
        ngayThanhToan: {
          $gte: start,
...
async function getPaymentReport(start: Date, end: Date, format: string) {
  const payments = await HoaDon.find({
    ngayThanhToan: {
      $gte: start,
      $lte: end,
    },
  ...
  const paymentStats = await HoaDon.aggregate([
    {
      $match: {
        ngayThanhToan: {
          $gte: start,
  ...
  const data = {
    ...
    totalAmount: payments.reduce((sum, p) => sum + p.tongTien, 0),
    payments,
    paymentStats,
  };
...
function generatePaymentCSV(data: any): string {
  ...
  csv += `Từ ngày: ${data.period.start}\n`;
  csv += `Đến ngày: ${data.period.end}\n`;
  csv += `Tổng số hóa đơn thanh toán: ${data.totalPayments}\n`;
  csv += `Tổng số tiền: ${data.totalAmount.toLocaleString('vi-VN')} VNĐ\n\n`;
  
  csv += 'Chi tiết thanh toán:\n';
  csv += 'Ngày thanh toán,Hóa đơn,Số tiền,Phương thức\n';
  data.payments.forEach((payment: any) => {
    const method = payment.phuongThucThanhToan === 'chuyenKhoan' ? 'Chuyển khoản' : payment.phuongThucThanhToan;
    csv += `${payment.ngayThanhToan.toISOString().split('T')[0]},${payment.maHoaDon},${payment.tongTien.toLocaleString('vi-VN')},${method}\n`;
  });
