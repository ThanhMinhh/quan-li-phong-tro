...
export async function PUT(
  ...
  try {
    ...
    if (session.user.role === 'nhanVien') {
      return NextResponse.json(
        { message: 'Nhân viên không có quyền chỉnh sửa thông tin tòa nhà' },
        { status: 403 }
      );
    }
    ...
    // Chỉ admin mới được đổi chủ sở hữu
    if (validatedData.chuSoHuu && session.user.role === 'admin') {
      updatePayload.chuSoHuu = validatedData.chuSoHuu;
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (session.user.role === 'nhanVien') {
      return NextResponse.json(
        { message: 'Nhân viên không có quyền xóa tòa nhà' },
        { status: 403 }
      );
    }
